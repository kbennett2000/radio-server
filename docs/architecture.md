...
| `Uvk5Radio` | `uvk5` | **Implemented and bench-proven** (built in ADR 0110–0114, brought up on hardware across ADR 0119–0132) — a Quansheng UV-K5/K6 via the AIOC, on nicsure's Dock firmware (classic, DP32G030) or our [V3 fork](https://github.com/kbennett2000/uv-k1-k5v3-firmware-custom) at F5+ (PY32F071); tuning/tone/mode/keying are BK4819 register writes (keying confirmed by read-back), audio is the AIOC's USB sound card. It is the only backend with a real RSSI busy line, PA read-back and `scan`. It does **not** advertise `set_power`: over the dock, power is a raw `0x36` PA-bias write whose per-band calibration the host cannot read (ADR 0128/0134). See [uvk5-setup.md](uvk5-setup.md). |
