...
## The first key is always lost (2026-07-26, latest)

[ADR 0140](adr/0140-the-first-key-is-always-lost.md). **Read this before trusting any RF number on
this bench, including the ones in ADRs 0138 and 0139.**

**The blocker.** The station keys **0/4 from cold** (≥2 min idle) and **4/4 warm**. An 8 s continuous
hold from cold radiates exactly as much as a 1.5 s over: nothing. It is not flaky in steady state —
`flaky_check` scored 10/10 back to back, 1.19–1.28 s.

Ruled out by measurement, so do not re-chase these:

| hypothesis | how it died |
|---|---|
| the radio moved frequency | `carrier_hunt.py` holds PTT and sweeps all 48 candidates in 400–480 MHz — on a silent over the carrier is **nowhere** |
| USB autosuspend | AIOC reports `power/control: on`, `autosuspend_delay_ms: -1000`, `runtime_suspended_time: 0` |
| the witness is deaf | a direct 20 Hz poll saw `busy` 21/27 across an over |
| the poller is dead | `watch_health.py`: 719 samples inside a 1.5 s window, thread alive, 0 errors |
...
and keeps exit 1 = measured failure distinct from 2 = inconclusive.

**ADR 0139's hand-tuned K0PRA acceptance is cancelled.** A repeater opens on carrier plus CTCSS, both
generated by the K5's own firmware from its front panel, so it would have measured the operator's
antenna and tone rather than radio-server. `repeater_openup.py` is not wasted — it is the right
acceptance for the hands-off run, once there is one.

**Mechanism, separately established.** `Dock_EnterFullControl` ends in `RADIO_SetupRegisters(true)`
(`App/app/uart.c:774`) → `BK4819_SetFrequency(gRxVfo->pRX->Frequency)` (`App/radio.c:807-809`). So
`0x0871` retunes from the radio's **own** VFO and **no dock write can survive it** — which is why 37
presets applied and read back correctly and still never keyed a machine. The end-to-end confirmation
did not complete: three attempts each died on their own positive control, killed by the cold-key
fault. That is the instrument working.

**The fix, built but never flashed.** Fork branch `f6-dock-set-vfo` adds opcode **`0x0873`**: hand the
radio a whole channel, then let its own `RADIO_ApplyOffset` → `RADIO_ConfigureSquelchAndOutputPower`
→ `RADIO_SetupRegisters` do the work, so `0x0871` stops being something to survive and becomes the
thing that applies it. Dispatched **outside** the full-control loop (that loop starves the main loop
which samples the PTT pin); **both** VFOs written (dual watch); malformed frames refused, not clamped.
Host tests 48/0, image builds clean (FLASH 57 %, RAM 64 %). Host side is `frames.SetVfo`, pinned to
the same byte vector as the firmware's tests.

**Also open:** `POST /radio/select` baofeng→uvk5 **segfaults the server** (139, 49 s in; `close()`
never runs so `0x0871` is never sent). Reachable from the web UI. ADR 0139 called that path safe on
the strength of the other direction only.

Next: the cold-key fault is the blocker for everything. Then flash F6 and prove a commanded channel.
