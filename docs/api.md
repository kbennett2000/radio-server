...
- **`409`** on a `uvk5` station whose frequency the host does not know (ADR 0174). In dock mode the
  host owns the synthesiser, so a radio whose registers do not read back a frequency is one nobody
  can say the transmitter would come up on — `GET /status` reports `frequency: null` rather than a
  number, and the key-up is refused before a register is written. Same remedy as every other
  not-ready `409`: `POST /frequency`, then resend unchanged. `POST /transmit` refuses identically —
  it self-keys through the same path — and so does `POST /split`, whose offset arithmetic would
  otherwise be measured against a receive frequency nobody knows.
- Not a `baofeng` behaviour, deliberately: there the **radio** holds the frequency (set by hand, or
  by a UV-K5 tuner), so a host that has not tuned is the ordinary post-restart state and the carrier
  goes out on the channel the operator dialled.
