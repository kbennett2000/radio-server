...
_BASE_SETTINGS: tuple[SettingSpec, ...] = (
    ...
    _s(
        ...
        "NOT work. An ALSA card id ('AIOC_K6' — the per-cable name a udev rule assigns off the USB "
        "serial) is also accepted and is the stable way to pick one of several identical AIOCs, "
        "which all share a PortAudio name (ADR 0124). The card is 48 kHz-native. If the default "
        "doesn't resolve, `python -m radio_server.doctor` prints the exact index/name to use.",
    ),
    _s(
        ...
        "Same matching rules and default as baofeng.input_device (name substring, integer index, or "
        "an ALSA card id — not a raw ALSA 'hw:' string).",
    ),
    _s(
    ...
    _s(
        ...
        "'All-In-One-Cable: USB' — the AIOC), an integer index, or an ALSA card id ('AIOC_K6' — the "
        "per-cable name a udev rule assigns off the USB serial, and the stable way to pick one of "
        "several identical AIOCs; ADR 0124). A raw ALSA 'hw:CARD=...' string does NOT work. The "
        "AIOC's K1 jack carries the radio's audio; plugging it in mutes the handheld's own "
        "speaker/mic (expected). Verify the exact name with `doctor --backend uvk5`.",
    ),
    _s(
